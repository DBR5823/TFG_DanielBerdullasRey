...
ARCHIVO_LOG="resultadosPruebasVIT/resultados_VIT+CUTMIX.log"

#Limpiamos el archivo si ya existe para guardar únicamente las nuevas estadísticas 
> "$ARCHIVO_LOG"

echo "********** Iniciando experimentos VIT+CUTMIX con aumentado: $(date +%T) **********" | tee -a "$ARCHIVO_LOG"
...
python3 -u vit_CutMix_pruebasParalelas.py 0 1 2>&1 | tee -a "$ARCHIVO_LOG"
sleep 2
...
do
    ...
    python3 -u vit_CutMix_pruebasParalelas.py "$i" 1 2>&1 | tee -a "$ARCHIVO_LOG"
    sleep 2
