...
def main(exp):
  ...
  # 6.1 Cross Entropy Loss, no mean-squared error
...
if __name__=='__main__':
  #Si el flag TEST está a 0 no se ejecuta el modelo sobre el conjunto de test para testearlo
  if(TEST==0): # validacion (se podria hacer una validacion cruzada de varias vias)
    acces=0
    #Se ejecuta la función main tantas veces como indique la variable EXP, en cada ejecución se suma la media del accuracy obtenida en ese experimento
    for exp in range(EXP): acces=acces+main(exp)
    print('* FINAL CNN21 SEG EXPERIMENTOS:',EXP,'EPOCHS:',EPOCHS,'SAMPLES:',SAMPLES,'ADA:',ADA,'AUM:',AUM)
    print('  Promedio FINAL de accuracy sobre VALIDACIÓN: %02.02f'%(100*acces/EXP))
  
  #Si el flag test está a 1 se realiza el test del modelo sobre el conjunto de test
  else: # test de 5 experimentos
    #Listas para guardar el Overal Accuracy, el Average Accuracy y las precisiones individuales para cada experimento
    OA=[0]*EXP; AA=[0]*EXP; aa=[0]*EXP 

    #Se ejecuta la función main tantas veces como indique la variable EXP y se almacenan los valores devueltos por los experimentos
    for exp in range(EXP): (OA[exp],AA[exp],aa[exp])=main(exp)
    
    print('*********************RESULTADOS FINALES*********************')
    #Si se ha realizado más de un experimento se calcula la media y la desviación típica para los resultados
