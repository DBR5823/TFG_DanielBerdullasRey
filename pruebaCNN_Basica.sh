...
python3 -u Codigos_AumentadoClaseMinoritaria/cnn21_pruebasParalelas.py 0 0 2>&1 | tee -a "$ARCHIVO_LOG"
...
do
    ...
    python3 -u Codigos_AumentadoClaseMinoritaria/cnn21_pruebasParalelas.py "$i" 0 2>&1 | tee -a "$ARCHIVO_LOG"
...
# 1. Ejecución inicial sin argumentos para ejecutar la prueba sobre el dataset de Oitaven
echo "Ejecutando: prueba inicial CNN Básica (sobre Oitaven)" | tee -a "$ARCHIVO_LOG"
python3 -u Codigos_AumentadoClaseMinoritaria/cnn21_pruebasParalelas.py 0 1 2>&1 | tee -a "$ARCHIVO_LOG"
...
do
    echo "Ejecutando prueba iteración: $i..." | tee -a "$ARCHIVO_LOG"
    # Ejecutamos el comando, redirigimos errores (stderr) al mismo lugar y usamos tee
    python3 -u Codigos_AumentadoClaseMinoritaria/cnn21_pruebasParalelas.py "$i" 1 2>&1 | tee -a "$ARCHIVO_LOG"
    echo "Prueba iteracion $i finalizada." | tee -a "$ARCHIVO_LOG"
    echo "********************************************" | tee -a "$ARCHIVO_LOG"
done
