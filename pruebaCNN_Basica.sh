...
echo "********** Iniciando experimentos CNN Básica: $(date +%T) **********" | tee -a "$ARCHIVO_LOG"
...
python3 -u cnn21_pruebasParalelas.py 2>&1 | tee -a "$ARCHIVO_LOG"
...
do
    ...
    python3 -u cnn21_pruebasParalelas.py "$i" 2>&1 | tee -a "$ARCHIVO_LOG"
