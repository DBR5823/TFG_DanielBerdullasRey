...
echo "********** Iniciando experimentos MIXUP: $(date +%T) **********" | tee -a "$ARCHIVO_LOG"
...
python3 -u cnn_21_MIXUP_PARALELO.py 2>&1 | tee -a "$ARCHIVO_LOG"
...
do
    ...
    python3 -u cnn_21_MIXUP_PARALELO.py "$i" 2>&1 | tee -a "$ARCHIVO_LOG"
